...
`assessPlantEnvironment` calcula VPD foliar y lo compara con una banda orientativa segun la etapa declarada. Si el usuario registra temperatura foliar, usa esa medicion junto con temperatura ambiental y humedad relativa. Si falta temperatura foliar, estima la hoja a 2,8 C por debajo del ambiente, el supuesto de la tabla de referencia incorporada, y lo etiqueta expresamente como estimado. El formulario muestra el VPD en vivo antes de guardar y la ficha presenta tendencias de temperatura, humedad y VPD usando exclusivamente las lecturas registradas, sin completar huecos. La interfaz diferencia la medicion original del valor `calculated`, muestra formula, fuente, banda utilizada y datos faltantes.
