...
15. Traduccion (en curso, por seccion): en `/en/today/` (o tocando "EN" en el selector desde Hoy), el encabezado, la navegacion, la cuenta, el onboarding, el paso a paso entre secciones y toda la seccion Hoy (metricas, tareas, clima, centro operativo, chequeo rapido, temporada, alertas ambientales, inventario, inspecciones, notificaciones push) se ven en ingles. Datos declarados por el usuario (nombres de plantas, etapas, tareas cargadas, fechas y el texto del clima) siguen en el idioma en que se cargaron: no son textos de interfaz, son contenido. En `/en/seeds/` el marco de la seccion Semillas ya esta en ingles (titulo, cartel legal, solapas, paso a paso entre solapas y el encabezado de "Mi cultivo"), y ademas ya esta traducido el contenido completo de las solapas Setups (sugerencias de armado, checklist manual, regla rapida de macetas) y Referencia (buscador de geneticas, ficha de cada genetica, campos originales del Excel, los avisos de "Field: ..." que indican a que campo copiar cada dato). Las solapas Finder, Mi cultivo y Calculadora todavia se ven en espanol, se traducen en un proximo parche. Las secciones Espacios, Calendario y Diario todavia se ven en espanol aunque el selector diga "EN"; se traducen en proximos parches.
