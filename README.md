...
`assessPlantEnvironment` calcula VPD foliar y lo compara con una banda orientativa segun la etapa declarada. Si el usuario registra temperatura foliar, usa esa medicion junto con temperatura ambiental y humedad relativa. Si falta temperatura foliar, estima la hoja a 2,8 C por debajo del ambiente, el supuesto de la tabla de referencia incorporada, y lo etiqueta expresamente como estimado. El formulario muestra el VPD en vivo antes de guardar y la ficha presenta tendencias de temperatura, humedad y VPD usando exclusivamente las lecturas registradas, sin completar huecos. La escala visual de bandas foliares se muestra tanto en el formulario como directamente en `Ambiente y sensores` de cada maceta. La interfaz diferencia la medicion original del valor `calculated`, muestra formula, fuente, banda utilizada y datos faltantes.

La ficha de cada maceta resume cuanto tiempo observado estuvo el VPD dentro de la banda de su etapa declarada durante los ultimos 30 dias. Solo suma intervalos entre lecturas consecutivas separados por hasta seis horas: no extrapola despues de la ultima lectura ni completa huecos mayores. Tambien muestra el dryback observado desde el ultimo riego registrado cuando existen al menos dos lecturas de humedad de sustrato, con curva, descenso, duracion y ritmo medio; describe datos y no prescribe un momento de riego.

Cada maceta puede declarar la hora de encendido y la cantidad de horas de luz. Con ese horario, Ambiente y sensores separa las lecturas calculables de los ultimos 30 dias entre luz y oscuridad, informa el VPD promedio de cada periodo y su diferencia. La clasificacion usa las fechas y horas registradas: no reconstruye el estado de una luminaria cuando falta el horario ni completa periodos sin mediciones.

El mapa PPFD permite cargar una cuadricula 2x2 o 3x3 a nivel de la copa. Cada punto se guarda como una medicion independiente de la maceta, y la interfaz calcula promedio, minimo, maximo y uniformidad `minimo / promedio x 100` sin interpolar zonas no medidas. El asistente foliar registra zona, patron, estado observado del sustrato, severidad y seguimiento; cruza ese registro con pH, EC, VPD y PPFD disponibles para proponer hipotesis que conviene comprobar. No diagnostica una carencia ni recomienda aplicar productos a partir de una fotografia o sintoma aislado.
...
El comparador de ciclos archivados contrasta configuracion declarada, duracion, peso seco, agua registrada, VPD promedio, cantidad de mediciones y notas. Solo calcula diferencias cuando ambos ciclos tienen el dato y enumera cambios de configuracion, sin atribuir causalidad ni rellenar faltantes.
